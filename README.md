...
|------|-----|------|-----|-----|----------|
| Pure Mamba | 35.69 | 5.97 | 4.80 | 0.5908 | 2.00 |
| **Pure MinGRU** | **30.51** | **5.52** | **4.39** | **0.6502** | **1.81** |
| Hybrid Mamba-GRU | 31.60 | 5.62 | 4.46 | 0.6376 | 1.84 |

**关键发现 (Negative Result)**:
- ✅ Pure MinGRU 表现最佳，参数效率高且在短序列上捕捉局部模式能力强
- ⚠️ 混合模型性能介于两者之间，但未能超越 Pure MinGRU
- 📊 劣化程度分析：混合模型相比 Pure MinGRU 性能下降 3.6%
- 💡 这表明在短序列（60步）场景下，简单的加权融合策略不足以发挥混合架构的优势
- 🔬 需要更智能的融合机制（如动态注意力）或更长的序列长度来体现 Mamba 的优势

*注: 诚实报告 negative results 对学术研究同样重要，有助于后续研究避免类似问题*

## 未来改进方向

基于当前的 negative results，提出以下改进方向：

- [ ] **动态融合机制**: 替代简单的加权融合，使用注意力机制根据输入动态调整融合权重
- [ ] **更长序列测试**: 在 seq_len > 120 的超长序列上验证 Mamba 的优势是否能够体现
- [ ] **分层融合**: 在不同层级进行融合，而不仅仅在输出层
- [ ] **多步预测扩展**: 测试混合架构在多步预测任务上的表现
- [ ] **特征级融合**: 在隐藏状态层面进行特征交互，而非简单加权
- [ ] **在线学习支持**: 支持增量学习和实时更新
